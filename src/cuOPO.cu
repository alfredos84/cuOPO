...
int main(int argc, char *argv[]){
	...
	real_t Temp		 = findPhaseMatchingTemp( lp, ls, Lambda, 10, 150 );  // signal wavelength [μm]
	real_t Lcr       = atof(argv[12])*1e3;  // crystal length [um]
	
	real_t np        = n(lp, Temp);         // pump ref. index
	real_t vp        = GV(lp, Temp); 		// pump group velocity [μm/ps]
	real_t b2p       = GVD(lp, Temp);       // pump GVD [ps²/μm] 
	real_t b3p       = 0.*TOD(lp, Temp);    // pump TOD [ps³/μm]	
	real_t kp        = 2*PI*dQ/(np*lp);   // pump kappa [1/V]
	
	real_t ns        = n(ls, Temp);         // signal ref. index
	real_t vs        = GV(ls, Temp); 		// signal group velocity [μm/ps]
	real_t b2s       = GVD(ls, Temp);       // signal GVD [ps²/μm] 
	real_t b3s       = 0.*TOD(ls, Temp);    // signal TOD [ps³/μm]		
	real_t ks        = 2*PI*dQ/(ns*ls);   // signal kappa [1/V]
	#ifdef THREE_EQS
	real_t ni        = n(li, Temp);         // idler ref. index
	real_t vi        = GV(li, Temp);		// idler group velocity [μm/ps]
	real_t b2i       = GVD(li, Temp);       // idler GVD [ps²/μm]
	real_t b3i       = 0.*TOD(li, Temp);    // idler TOD [ps³/μm]	
	real_t ki        = 2*PI*dQ/(ni*li);   // idler kappa [1/V]
	#endif
	...
	real_t Lcav      = 100e3;                             // cavity length [um]
	...
	if (!is_Ai_resonant){
		...
		Ith   = EPS0*C*np*ns*ni*ls*li*powf((1/dQ/Lcr/PI),2)*alphas/2;
	}
	if (!is_As_resonant){
		std::cout << "SRO, Ai is resonant" << std::endl;
		Ith   = EPS0*C*np*ns*ni*ls*li*powf((1/dQ/Lcr/PI),2)*alphai/2;
	}
	if (is_As_resonant and is_Ai_resonant){
		std::cout << "DRO, As and Ai are resonant" << std::endl;
		Ith   = EPS0*C*np*ns*ni*ls*li*powf((1/dQ/Lcr/PI),2)*alphas*alphai/8;
	}
	Pth   = Ith*spot;
	#else
	// Power and intensity threshold degenerate DRO 
	Ith   = EPS0*C*np*powf((ns*ls*alphas/dQ/Lcr/PI), 2)/8;
	Pth   = Ith*spot;
	#endif
	...
	#ifdef THREE_EQS
	real_t GammaL = 2*PI*dQ*Ap0*Lcr/sqrtf(ns*ni*ls*li);	// Gain regime Γ.L
	#else
	real_t GammaL = 2*PI*dQ*Ap0*Lcr/(ns*ls);	// Gain regime Γ.L
	#endif
	...
	if ( print_param_on_screen ){
		...
		std::cout << "Net TOD                 = " << (1-0.01*atoi(argv[8]))*b3s << " ps³/\u03BCm"  << std::endl;
		...
		std::cout << "(\u0393L)²                   = " << GammaL*GammaL << std::endl;
		// std::cout << "sinh²(\u0393L)               = " << powf(sinh(GammaL),2) << std::endl;
		if(using_phase_modulator){
	...
	if(abs(PI*fpm*mod_depth*dkp*Lcr/alphas) >= sqrtf(Nth-1)){
		std::cout << "A single pulse is expected: " << std::endl;
		std::cout << "Valor = " << abs(sinf(PI*fpm*dkp*Lcr)*mod_depth/alphas) << ">=" << sqrtf(Nth-1) << std::endl;
	}
	else{
		std::cout << "A multipulse structure is expected: " << std::endl;
		std::cout << "Valor = " << abs(sinf(PI*fpm*dkp*Lcr)*mod_depth/alphas) << "!>=" << sqrtf(Nth-1) << std::endl;
	}
	...
	for (uint nn = 0; nn < NRT; nn++){
		...
		if(GDD!=0){ // adds dispersion compensation
			...
			CHECK(cudaDeviceSynchronize());
			if(TODscomp!=0 and b3s!=0){
				AddGDDTOD<<<grid,block>>>(Asw_gpu, auxs_gpu, w_gpu, GDD, TODscomp);
				CHECK(cudaDeviceSynchronize());
			}
			else{
			...
			CHECK(cudaDeviceSynchronize());
			if(TODicomp!=0 and b3i!=0){
				AddGDDTOD<<<grid,block>>>(Aiw_gpu, auxi_gpu, w_gpu, GDD, TODicomp);
				CHECK(cudaDeviceSynchronize());
			}
			else{
		...
		// ADDITIONAL PHASE AND CAVITY LOSSES
		if (is_As_resonant){ // if As is resonant, adds phase and losses
			AddPhase<<<grid,block>>>( As_gpu, auxs_gpu, Rs, delta );
			CHECK(cudaDeviceSynchronize());
		}		
		#ifdef THREE_EQS
		if (is_Ai_resonant){  // if Ai is resonant, adds phase and losses
			AddPhase<<<grid,block>>>( Ai_gpu, auxi_gpu, Ri, delta );
			CHECK(cudaDeviceSynchronize());
		}
		#endif
