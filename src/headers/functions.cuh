...
/** Computes the phase-matching temperature for a given	grating period, Lambda,
	in an interval of temperatures [T_min, T_max] for SHG*/
real_t findPhaseMatchingTemp( real_t lp, real_t ls, real_t Lambda, real_t T_min, real_t T_max )
{
	uint m = 5000;
	uint i;
	...
	real_t *T  = (real_t*) malloc(m*sizeof(real_t));
	real_t *dk = (real_t*) malloc(m*sizeof(real_t));

	linspace( T, m, T_min, T_max);

	for ( i = 0; i < m; i++ ){
		// Please check the involved TWM process
		dk[i] = 2*PI/lp*abs(n(ls,T[i]) - n(lp,T[i])) - 2*PI/Lambda ;
	}

	// find the minimum
	int pos = 0; i = 1;
	while ( i < m ){
		if ( abs(dk[i]) < abs(dk[pos]) ){ 
			pos = i;
		}
		i++;
	}
...
__global__ void AddPhase(complex_t *A, complex_t *aux, real_t R, real_t delta)
{
	
	uint idx = threadIdx.x + blockIdx.x * blockDim.x;
	
	if (idx < SIZE){
		aux[idx] = sqrtf(R) * A[idx] * CpxExp(PI*delta);
	}
	if (idx < SIZE){
		A[idx] = aux[idx];
	}
...
__global__ void AddGDDTOD(complex_t *A, complex_t *aux, real_t *w, real_t GDD,  real_t TOD)
{
	...
	if (idx < SIZE){
		aux[idx] = A[idx] * CpxExp(0.5*w[idx] * (w[idx]*GDD + w[idx]*w[idx]*TOD/3.0) );
	}
	if (idx < SIZE){
		A[idx] = aux[idx];
	}	
